...
//
//  Created by Daniil Zadorozhnyy on 17.08.2022.
//
...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadFeedActions_requestFeedFromLoader() {
        ...
        sut.loadViewIfNeeded()
        XCTAssertEqual(loader.loadFeedCallCount, 1, "Expected a loading request oncce view is loaded")
        ...
        sut.simulateUserInitiatedReload()
        XCTAssertEqual(loader.loadFeedCallCount, 3, "Expected a thirs loading request once user initiates another load")
    }
    ...
    func test_loadMoreIndicator_isVisibleWhileLoadingMore() {
        ...
        sut.loadViewIfNeeded()
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected load more indicator once view is loaded")
        
        loader.completeFeedLoading(at: 0)
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no load more indicator once loading completes successfully")
        
        sut.simulateLoadMoreFeedAction()
        XCTAssertTrue(sut.isShowingLoadMoreFeedIndicator, "Expected load more indicator on load more action")
    
        loader.completeLoadMore(at: 0)
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no load more indicator once user initiated loading completes successfully")
        
        sut.simulateLoadMoreFeedAction()
        XCTAssertTrue(sut.isShowingLoadMoreFeedIndicator, "Expected load more indicator on second  load more action")
        
        loader.completeLoadMoreWithError(at: 1)
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no load more indicator once user initiated loading completes with error")
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no load more indicator once user initiated loading completes with error")
    }
    ...
    func test_feedImageView_loadsImageURLWhenVisible() {
        let image0 = makeImage(url: URL(string: "http://url-0.come")!)
        let image1 = makeImage(url: URL(string: "http://url-1.come")!)
        let (sut, loader) = makeSUT()
    ...
    func test_feedImageView_rendersImageLoadedFromURL() {
        ...
        loader.completeImageLoading(with: imageData0, at: 1)
    ...
    func test_feedImageViewRetryButton_isVisibleOnImageURLLoadError() {
        ...
        XCTAssertEqual(view1?.isShowingRetryAction, true, "Expected retry action state change for second view once second image loading completes with error")
    }
    ...
    func test_feedImageViewRetryButton_isVisibleOnInvalidImageData() {
        ...
        XCTAssertEqual(view?.isShowingRetryAction, false, "Expected no retry action for view while loading image")
        
        let imageData = Data("invalid image data".utf8)
        loader.completeImageLoading(with: imageData, at: 0)
        
        XCTAssertEqual(view?.isShowingRetryAction, true, "Expected retry action for view once image loading completes with invalid image data")
    }
    ...
    func test_feedImageView_doesNotRenderLoadedImageWhenNotVisibleAnymore() {
        ...
        XCTAssertNil(view?.renderedImage, "Expected no rendered image when image load finishes after the view is not visbile anymore")
    }
    ...
    func test_loadImageDataCompletion_dispatchedFromBackgroundToMainThred() {
        let (sut, loader) = makeSUT()
        sut.loadViewIfNeeded()
    ...
    func test_feedImageView_doesNotLoagImageAgainUntilPreviousRequestCompletes()  {
        ...
        loader.completeImageLoading(at: 0)
        sut.simulateFeedImageViewVisible(at: 0)
        XCTAssertEqual(loader.loadedImageURLs, [image.url, image.url], "Expected second request when visible after previous request completes")
